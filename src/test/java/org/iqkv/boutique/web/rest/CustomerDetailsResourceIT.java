...
class CustomerDetailsResourceIT {
    ...
    void createCustomerDetails() throws Exception {
        ...
        var returnedCustomerDetails = om.readValue(
            restCustomerDetailsMockMvc
                .perform(
                    post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerDetails))
                )
    ...
    void putWithMissingIdPathParamCustomerDetails() throws Exception {
        ...
        restCustomerDetailsMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerDetails))
            )
