...
class ShoppingCartResourceIT {
    ...
    void createShoppingCart() throws Exception {
        ...
        var returnedShoppingCart = om.readValue(
            restShoppingCartMockMvc
                .perform(
                    post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shoppingCart))
                )
    ...
    void createShoppingCartWithExistingId() throws Exception {
        ...
        restShoppingCartMockMvc
            .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shoppingCart)))
    ...
    void putWithMissingIdPathParamShoppingCart() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShoppingCartMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shoppingCart)))
    ...
    void patchWithMissingIdPathParamShoppingCart() throws Exception {
        ...
        restShoppingCartMockMvc
            .perform(
                patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(shoppingCart))
            )
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteShoppingCart() throws Exception {
        ...
        restShoppingCartMockMvc
            .perform(delete(ENTITY_API_URL_ID, shoppingCart.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
